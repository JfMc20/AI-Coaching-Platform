...
from .program_engine import (
    StepHandler, TriggerHandler, ActionHandler, 
    ExecutionContext, StepExecutionResult, DebugSession, StepDebugSession
)
from .step_models import (
    ProgramStep, TriggerConfig, ActionConfig, ValidationConfig,
...
class PersonalityIntegration:
    ...
    async def enhance_step(
        ...
        try:
            ...
            analysis_response = await self.personality_engine.analyze_creator_personality(
            ...
            if not analysis_response.personality_profile:
                debug.log_milestone("no_personality_profile", "Using default personality")
                return {"personality_available": False}
            
            # Generate personalized prompt if content-based step
            enhanced_content = None
            if step.action_config.action_type in [ActionType.SEND_MESSAGE, ActionType.ASSIGN_TASK]:
                ...
                prompt_response = await self.prompt_generator.generate_personalized_prompt(
            ...
            debug.log_milestone("personality_enhancement_completed", {
    ...
    async def validate_response_consistency(
        ...
        try:
            ...
            if analysis_response.personality_profile:
                monitoring_response = await self.consistency_monitor.monitor_response_consistency(
            ...
            return {"consistency_check_performed": False, "reason": "No personality profile"}
...
class KnowledgeIntegration:
    ...
    async def enhance_step(
        ...
        try:
            ...
            if step.action_config.action_type not in [ActionType.SEND_MESSAGE, ActionType.ASSIGN_TASK]:
                return {"knowledge_available": False, "reason": "Not content-based step"}
            ...
            if not knowledge_context.get("knowledge_chunks"):
                ...
                return {"knowledge_available": False, "reason": "No relevant knowledge found"}
            ...
            for chunk in knowledge_context["knowledge_chunks"][:3]:  # Top 3 chunks
                knowledge_snippets.append({
                    "content": chunk["content"][:200] + "...",  # Limit snippet length
            ...
            debug.log_milestone("knowledge_enhancement_completed", {
    ...
    def _build_knowledge_query(self, step: ProgramStep, context: ExecutionContext) -> str:
        """Build knowledge query from step context"""
        query_parts = []
    ...
    async def _generate_knowledge_enhanced_content(
        ...
        try:
            ...
            base_content = step.action_config.dynamic_content or step.description or step.title
...
class AIEnhancement:
    ...
    async def enhance_step(
        ...
        try:
            ...
            if not personality_data.get("personality_available") and not knowledge_data.get("knowledge_available"):
                return {"ai_enhancement_applied": False, "reason": "No base enhancements available"}
            ...
            enhancement = {
                ...
                "personalization_level": ai_enhancement.get("personalization_score", 0.5)
            }
            
            debug.record_enhancement("ai", enhancement)
            debug.log_milestone("ai_enhancement_completed", {
    ...
    def _create_enhancement_prompt(
        ...
        prompt_parts = [
            f"Enhance this coaching step for maximum effectiveness:",
            f"",
            f"Step: {step.title}",
    ...
    def _parse_ai_enhancement(self, ai_response: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "content": ai_response[:500],  # First 500 chars as content
                "improvements": [],
                "engagement_tips": [],
...
class EnhancedProgramStep:
    ...
    def __init__(self, base_step: ProgramStep, enhancements: Dict[str, Any], enhancement_metadata: Dict[str, Any]):
    ...
    def get_enhanced_content(self) -> str:
        ...
        return (
            self.base_step.action_config.dynamic_content or 
            self.base_step.description or 
            self.base_step.title
        )
    ...
    def get_quality_indicators(self) -> Dict[str, Any]:
        ...
        return {
            "personality_enhanced": bool(self.enhancements.get("personality", {}).get("personality_available")),
            "knowledge_enhanced": bool(self.enhancements.get("knowledge", {}).get("knowledge_available")),
            "ai_enhanced": bool(self.enhancements.get("ai", {}).get("ai_enhancement_applied")),
...
class StepProcessor:
    ...
    def register_step_handler(self, step_type: StepType, handler: StepHandler):
        ...
        logger.info(f"Registered custom handler for {step_type}: {handler.__class__.__name__}")
    ...
    async def process_step(
        ...
        try:
            step_debug.log_milestone("step_processing_started", {
            ...
            step_debug.log_milestone("handler_selected", {
                "handler_class": handler.__class__.__name__,
                "can_handle": await handler.can_handle(step)
            })
            
            # Step 3: Execute step with handler
            execution_result = await handler.execute(enhanced_step.base_step, context, step_debug)
            ...
            if enhanced_result.success and enhanced_result.result_data.get("response_content"):
                consistency_data = await self.personality_integration.validate_response_consistency(
            ...
            enhanced_result.execution_time_seconds = (enhanced_result.completed_at - start_time).total_seconds()
            
            # Step 7: Calculate quality scores
            quality_scores = self._calculate_quality_scores(enhanced_step, enhanced_result)
            enhanced_result.personality_consistency = quality_scores.get("personality_consistency", 0.0)
            enhanced_result.engagement_score = quality_scores.get("engagement_score", 0.0)
            enhanced_result.success_score = quality_scores.get("success_score", 0.0)
            
            step_debug.log_success("step_processing_completed", {
    ...
    async def _enhance_execution_result(
        ...
        result.result_data["quality_indicators"] = enhanced_step.get_quality_indicators()
        result.result_data["personalization_score"] = enhanced_step.get_personalization_score()
        
        # Add enhanced content to result
        if enhanced_step.get_enhanced_content():
            result.result_data["enhanced_content"] = enhanced_step.get_enhanced_content()
        
        # Add improvement suggestions
        improvements = []
        for enhancement_type, enhancement_data in enhanced_step.enhancements.items():
            if isinstance(enhancement_data, dict) and "suggested_improvements" in enhancement_data:
    ...
    def _calculate_quality_scores(
        ...
        if personality_data.get("personality_available"):
            scores["personality_consistency"] = personality_data.get("personality_confidence", 0.5)
        else:
        ...
        scores["engagement_score"] = min(1.0, (enhancement_count * 0.3) + (enhancement_quality * 0.7))
...
class MessageStepHandler(StepHandler):
    ...
    async def execute(
        ...
        message_content = (
            step.action_config.dynamic_content or 
            step.description or 
            f"Message from step: {step.title}"
        )
        
        # Simulate channel delivery
        preferred_channel = step.action_config.preferred_channels[0] if step.action_config.preferred_channels else ChannelType.WEB_WIDGET
        
        debug_session.log_milestone("message_sent", {
...
class TaskStepHandler(StepHandler):
    ...
    async def execute(
        ...
        task_instructions = step.action_config.task_instructions or step.description or f"Complete task: {step.title}"
        
        # Simulate task assignment
        task_id = f"task_{context.execution_id}_{step.step_id}"
        
        debug_session.log_milestone("task_assigned", {
            "task_id": task_id,
            "instructions_length": len(task_instructions)
        })
...
class EvaluationStepHandler(StepHandler):
    ...
    async def execute(
        ...
        debug_session.log_milestone("evaluation_created", {
...
class CheckpointStepHandler(StepHandler):
    ...
    async def execute(
        ...
        total_steps = len(context.completed_steps) + len(context.active_steps) + len(context.failed_steps)
        progress_percentage = (len(context.completed_steps) / max(total_steps, 1)) * 100
        
        checkpoint_message = f"Checkpoint reached: {step.title}. Progress: {progress_percentage:.1f}%"
        
        debug_session.log_milestone("checkpoint_reached", {
...
class ConditionalStepHandler(StepHandler):
    ...
    async def execute(
        ...
        if not step.conditional_config:
            raise ValueError("Conditional step requires conditional_config")
        ...
        if condition_result:
            next_steps = step.conditional_config.true_path_step_ids
            path_taken = "true_path"
        else:
            next_steps = step.conditional_config.false_path_step_ids
            path_taken = "false_path"
        
        debug_session.log_milestone("condition_evaluated", {
            "condition": step.conditional_config.condition_expression,
        ...
        return StepExecutionResult(
            ...
            result_data={
                ...
                "condition_expression": step.conditional_config.condition_expression,
                "condition_result": condition_result,
                "path_taken": path_taken,
                "response_content": f"Condition evaluated: {path_taken}"
            },
            next_step_recommendations=next_steps,
            conditional_outcomes={"condition_result": condition_result, "path": path_taken},
            success_score=1.0,
            engagement_score=0.5
        )
    
    def _evaluate_simple_condition(self, expression: str, context: ExecutionContext) -> bool:
        """Simple condition evaluation (placeholder)"""
    ...
    async def validate(self, step: ProgramStep) -> List[str]:
        ...
        elif not step.conditional_config.condition_expression:
...
class DelayStepHandler(StepHandler):
    ...
    async def execute(
        ...
        delay_seconds = step.trigger_config.delay_seconds or 60  # Default 1 minute
        
        # In real implementation, this would schedule future execution
        # For now, we'll simulate immediate completion
        debug_session.log_milestone("delay_scheduled", {
    ...
    async def validate(self, step: ProgramStep) -> List[str]:
        issues = []
        if step.trigger_config.delay_seconds and step.trigger_config.delay_seconds <= 0:
