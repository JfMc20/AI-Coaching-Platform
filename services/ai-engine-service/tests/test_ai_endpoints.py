...
class TestAIEndpoints:
    ...
    async def test_embedding_endpoint_success(self, client: AsyncClient, test_embedding_request):
        """Test successful embedding generation."""
        response = await client.post("/api/v1/ai/ollama/test-embedding", json=test_embedding_request)
    ...
    async def test_embedding_endpoint_validation(self, client: AsyncClient):
        """Test embedding endpoint input validation."""
        # Test empty text
        response = await client.post("/api/v1/ai/ollama/test-embedding", json={"text": ""})
        assert response.status_code == 422
        
        # Test missing text field
        response = await client.post("/api/v1/ai/ollama/test-embedding", json={})
        assert response.status_code == 422
        
        # Test invalid JSON
        response = await client.post("/api/v1/ai/ollama/test-embedding", data="invalid json")
        assert response.status_code == 422

    async def test_chat_endpoint_success(self, client: AsyncClient, test_chat_request):
        """Test successful chat completion."""
        response = await client.post("/api/v1/ai/ollama/test-chat", json=test_chat_request)
    ...
    async def test_chat_endpoint_minimal_request(self, client: AsyncClient):
        """Test chat endpoint with minimal required data."""
        minimal_request = {"message": "Hello, how are you?"}
        
        response = await client.post("/api/v1/ai/ollama/test-chat", json=minimal_request)
        assert response.status_code == 200
    ...
    async def test_chat_endpoint_with_context(self, client: AsyncClient):
        ...
        response = await client.post("/api/v1/ai/ollama/test-chat", json=request_with_context)
        assert response.status_code == 200
    ...
    async def test_chat_endpoint_validation(self, client: AsyncClient):
        ...
        # Test empty message
        response = await client.post("/api/v1/ai/ollama/test-chat", json={"message": ""})
        assert response.status_code == 422
        
        # Test missing message field
        response = await client.post("/api/v1/ai/ollama/test-chat", json={})
        assert response.status_code == 422

    async def test_embedding_consistency(self, client: AsyncClient):
        """Test that same input produces consistent embeddings."""
        test_text = {"text": "Consistent embedding test text"}
        
        # Generate embedding twice
        response1 = await client.post("/api/v1/ai/ollama/test-embedding", json=test_text)
        response2 = await client.post("/api/v1/ai/ollama/test-embedding", json=test_text)
    ...
    async def test_large_text_embedding(self, client: AsyncClient, performance_test_data):
        """Test embedding generation with large text."""
        large_text_request = {"text": performance_test_data["long_text"]}
        
        response = await client.post("/api/v1/ai/ollama/test-embedding", json=large_text_request)
    ...
    async def test_special_characters_handling(self, client: AsyncClient):
        ...
        # Test embedding
        response = await client.post("/api/v1/ai/ollama/test-embedding", json=special_text)
        assert response.status_code == 200
        ...
        response = await client.post("/api/v1/ai/ollama/test-chat", json=chat_request)
        assert response.status_code == 200

    async def test_concurrent_ai_requests(self, client: AsyncClient, performance_test_data):
        """Test concurrent AI requests."""
        import asyncio
        
        # Create concurrent embedding requests
        embedding_tasks = [
            client.post("/api/v1/ai/ollama/test-embedding", json={"text": text})
            for text in performance_test_data["concurrent_requests"][:5]
        ]
        
        # Create concurrent chat requests
        chat_tasks = [
            client.post("/api/v1/ai/ollama/test-chat", json={"message": text})
            for text in performance_test_data["concurrent_requests"][:5]
        ]
    ...
    async def test_ai_response_quality(self, client: AsyncClient, mock_ai_responses):
    ...
    async def test_ai_error_handling(self, client: AsyncClient):
        """Test AI service error handling."""
        # Test with extremely long text that might cause issues
        very_long_text = {"text": "A" * 100000}  # 100k characters
        
        response = await client.post("/api/v1/ai/ollama/test-embedding", json=very_long_text)
    ...
    async def test_model_availability(self, client: AsyncClient):
        ...
        response = await client.post("/api/v1/ai/ollama/test-embedding", json=model_request)
