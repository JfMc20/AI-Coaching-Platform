...
class ModelManager:
    ...
    async def deploy_model_version(
        ...
        try:
            logger.info(
                f"Starting {strategy.value} deployment of {model_name}:{version}"
            )
    ...
    async def _canary_deployment(
        self, new_version: ModelVersion, current_version: Optional[str]
    ) -> bool:
        """
        ...
        try:
            ...
            for stage_percent in canary_stages:
                logger.info(
                    f"Canary stage: {stage_percent}% traffic to {new_version.version}"
                )

                # Configure traffic split
                await self._configure_traffic_split(
                    new_version.model_name, new_version.version, stage_percent
                )

                # Monitor for rollback conditions
                monitoring_duration = (
                    300 if stage_percent < 100 else 600
                )  # 5-10 minutes
                await asyncio.sleep(monitoring_duration)

                # Check performance metrics
                metrics = await self._collect_performance_metrics(
                    new_version.model_name, new_version.version
                )

                # Evaluate rollback conditions
                should_rollback = await self._should_rollback(
                    metrics, new_version.rollback_threshold
                )

                if should_rollback:
                    logger.warning(f"Rollback triggered at {stage_percent}% stage")
                    await self._rollback_to_previous_version(
                        new_version.model_name, current_version
                    )
                    return False
    ...
    async def _blue_green_deployment(
        ...
        try:
            ...
            await self._configure_traffic_split(
                new_version.model_name, new_version.version, 100
            )
    ...
    async def _rolling_deployment(
        ...
        try:
            ...
            for stage in rollout_stages:
                ...
                await self._configure_traffic_split(
                    new_version.model_name, new_version.version, stage
                )
        ...
        try:
            for metric_name, threshold in thresholds.items():
                ...
                if metric_name == "error_rate" and current_value > threshold:
                    logger.warning(
                        f"Error rate {current_value} exceeds threshold {threshold}"
                    )
                    return True
                elif metric_name == "latency_p95" and current_value > threshold:
                    logger.warning(
                        f"P95 latency {current_value}ms exceeds threshold {threshold}ms"
                    )
                    return True
                elif metric_name == "availability" and current_value < threshold:
                    logger.warning(
                        f"Availability {current_value} below threshold {threshold}"
                    )
                    return True
    ...
    async def _rollback_to_previous_version(
        self, model_name: str, previous_version: Optional[str]
    ) -> bool:
        """
        ...
        try:
            if not previous_version:
                logger.error(
                    f"No previous version available for rollback of {model_name}"
                )
                return False
    ...
    async def _store_model_version(self, model_version: ModelVersion) -> None:
        ...
        await self.cache_manager.set(
            cache_key, json.dumps(version_data), ttl=86400 * 7
        )  # 7 days

    async def _configure_traffic_split(
        self, model_name: str, version: str, percentage: int
    ) -> None:
        """
    ...
    async def _run_smoke_tests(self, model_name: str, version: str) -> bool:
        ...
        try:
            ...
            if model_name == "embedding":
                ...
                test_response = await ollama_manager.generate_chat_completion(
                    "Hello, test"
                )
