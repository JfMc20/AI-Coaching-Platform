...
def _parse_iso_datetime(date_string: str) -> datetime:
    """
    Robust ISO datetime parsing that handles various formats

    Args:
        date_string: ISO datetime string in various formats
    ...
    # List of common ISO datetime formats to try
    formats_to_try = [
        # Standard ISO formats
        "%Y-%m-%dT%H:%M:%S.%fZ",  # 2023-12-01T10:30:45.123456Z
        "%Y-%m-%dT%H:%M:%SZ",  # 2023-12-01T10:30:45Z
        "%Y-%m-%dT%H:%M:%S.%f%z",  # 2023-12-01T10:30:45.123456+00:00
        "%Y-%m-%dT%H:%M:%S%z",  # 2023-12-01T10:30:45+00:00
        "%Y-%m-%dT%H:%M:%S.%f",  # 2023-12-01T10:30:45.123456
    ...
    # First, try the simple Z replacement approach (most common)
    try:
    ...
    # If that fails, try various explicit formats
    ...
    # If all formats fail, try to parse with dateutil as last resort
    try:
    ...
    raise ValueError(
        f"Unable to parse datetime string '{date_string}'. "
        f"Expected ISO format like '2023-12-01T10:30:45Z' or '2023-12-01T10:30:45.123456+00:00'"
    )
...
def serialize_message(message: Message) -> Dict[str, Any]:
    ...
    # Convert datetime fields to ISO strings - only if they are datetime instances
    if message_dict.get("created_at") and isinstance(message.created_at, datetime):
        message_dict["created_at"] = message.created_at.isoformat()

    if message_dict.get("updated_at") and isinstance(message.updated_at, datetime):
...
def deserialize_message(message_data: Dict[str, Any]) -> Message:
    ...
    Args:
        message_data: Dictionary with message data (not modified)
    ...
    # Parse datetime fields from ISO strings with robust parsing
    if isinstance(data_copy.get("created_at"), str):
        data_copy["created_at"] = _parse_iso_datetime(data_copy["created_at"])

    if isinstance(data_copy.get("updated_at"), str):
        data_copy["updated_at"] = _parse_iso_datetime(data_copy["updated_at"])
...
class ConversationManager:
    ...
    def __init__(self, cache_manager=None, max_context_messages: int = 20):
        ...
        logger.info(
            f"ConversationManager initialized with max_context_messages={max_context_messages}"
        )
    ...
    async def get_context(
        self, conversation_id: str, max_messages: int = 10, creator_id: str = "system"
    ) -> List[Message]:
        """
    ...
    @trace_ml_operation(
        "conversation_add_exchange", operation_type=OperationType.CONVERSATION
    )
    async def add_exchange(
        ...
        try:
            ...
            cache_key = f"conversation:{conversation_id}:messages"
            current_messages = (
                await self.cache_manager.redis.get(creator_id, cache_key) or []
            )

            # Add new messages using centralized serialization
            new_messages = current_messages + [
                serialize_message(user_msg),
                serialize_message(ai_msg),
            ]
            ...
            await self.cache_manager.redis.set(
                creator_id, cache_key, new_messages, self.context_ttl
            )
            ...
            if (
                len(self._conversation_storage[conversation_id])
    ...
    async def get_conversation_summary(
        self, conversation_id: str, creator_id: str = "system"
        ...
        try:
            messages = await self.get_context(
                conversation_id, max_messages=100, creator_id=creator_id
            )
            ...
            return {
                ...
                "first_message": messages[0].created_at.isoformat()
                if messages
                else None,
                "last_message": messages[-1].created_at.isoformat()
                if messages
                else None,
                "avg_response_time_ms": avg_response_time,
                "avg_response_length": sum(len(m.content) for m in ai_messages)
                / len(ai_messages)
                if ai_messages
                else 0,
            }
...
class RAGPipeline:
    ...
    async def process_query(
        ...
        try:
            logger.info(
                f"Processing query for creator {creator_id}, conversation {conversation_id} [correlation_id: {correlation_id}]"
            )
            ...
            # 4. Generate response
            # Use full contextual prompt for better responses, with fallback to simple prompt
            simple_prompt = f"User: {query}\nAssistant:"

            try:
                logger.info(
                    f"Using full contextual prompt (length: {len(prompt)} chars)"
                )
                logger.debug(f"Full prompt preview: {prompt[:200]}...")
                ...
                logger.info(
                    "Successfully generated response using full contextual prompt"
                )

            except Exception as e:
                logger.warning(
                    f"Failed to generate response with full prompt: {e}. "
                    f"Falling back to simple prompt."
                )
                logger.info(f"Using simple prompt fallback: {simple_prompt[:100]}...")

                try:
                    chat_response = await self.ollama_manager.generate_chat_response(
                        prompt=simple_prompt, temperature=0.7, max_tokens=200
                    )
                    logger.info(
                        "Successfully generated response using simple prompt fallback"
                    )

                except Exception as fallback_error:
                    logger.error(
                        f"Both full and simple prompt generation failed: {fallback_error}"
                    )
                    raise RAGError(
                        f"Chat response generation failed with both full and simple prompts: {fallback_error}"
                    )

            # 5. Calculate confidence score
            confidence = self.calculate_confidence_score(
                relevant_chunks, chat_response.response
            )
            ...
            ml_metrics.token_count = (
                chat_response.processing_time_ms or 0
    ...
    async def retrieve_knowledge(
        self, query: str, creator_id: str, limit: int = 5
    ) -> List[RetrievedChunk]:
        """
        ...
        try:
            ...
            logger.debug(
                f"Retrieved {len(chunks)} relevant chunks for query using optimized search"
            )
            return chunks
    ...
    async def build_contextual_prompt(
        ...
        try:
            ...
            prompt_parts.extend(["", "CONVERSATION HISTORY:"])
            ...
            prompt_parts.extend(["", f"User: {query}", "Assistant:"])
    ...
    def truncate_prompt_intelligently(self, prompt: str, max_tokens: int) -> str:
        """
        ...
        try:
            ...
            max_chars = max_tokens * 4
            return (
                prompt[: max_chars - 3] + "..." if len(prompt) > max_chars else prompt
            )

    def calculate_confidence_score(
        self, chunks: List[RetrievedChunk], response: str
    ) -> float:
        """
        ...
        try:
            ...
            avg_similarity = sum(chunk.similarity_score for chunk in chunks) / len(
                chunks
            )
    ...
    async def get_pipeline_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "p95_processing_time_ms": processing_times[int(total_queries * 0.95)]
                if total_queries > 0
                else 0,
