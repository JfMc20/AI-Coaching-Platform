...
class TestAuthEndpoints:
    ...
    async def test_user_registration_duplicate_email(self, client: AsyncClient, test_user_data):
    ...
    async def test_user_login_success(self, client: AsyncClient, registered_user, test_user_data):
    ...
    async def test_user_login_invalid_credentials(self, client: AsyncClient, registered_user, test_user_data):
    ...
    async def test_token_refresh_success(self, client: AsyncClient, authenticated_user):
    ...
    async def test_token_validation_success(self, client: AsyncClient, authenticated_user):
    ...
    async def test_token_validation_invalid_token(self, client: AsyncClient):
    ...
    async def test_token_validation_missing_token(self, client: AsyncClient):
        """Test token validation without token."""
        response = await client.get("/api/v1/auth/validate")
        assert response.status_code == 401

    async def test_password_reset_request(self, client: AsyncClient, registered_user, test_user_data):
    ...
    async def test_user_profile_access(self, client: AsyncClient, authenticated_user):
    ...
    async def test_concurrent_logins(self, client: AsyncClient, registered_user, test_user_data):
    ...
    async def test_rate_limiting(self, client: AsyncClient, test_user_data):
    ...
    async def test_tenant_isolation(self, client: AsyncClient, multiple_registered_users, test_users_data):
        """Test that users from different tenants are isolated."""
        # Login users from different tenants
        user1_login = await client.post("/api/v1/auth/login", json={
            "email": test_users_data[0]["email"],
            "password": test_users_data[0]["password"]
        })
        
        user2_login = await client.post("/api/v1/auth/login", json={
        ...
        user1_profile = await client.get("/api/v1/auth/validate", headers=user1_headers)
        user2_profile = await client.get("/api/v1/auth/validate", headers=user2_headers)
