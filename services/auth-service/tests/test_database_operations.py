...
class TestDatabaseOperations:
    ...
    async def test_concurrent_database_operations(self, test_engine):
        ...
        async def create_user(session, email, tenant_id):
            user = User(
                email=email,
                password_hash="hashed_password",
        ...
        # Create multiple sessions and users concurrently
        tasks = []
        for i in range(5):
            session = async_session()
            task = create_user(session, f"concurrent{i}@example.com", f"tenant-{i}")
            tasks.append(task)
